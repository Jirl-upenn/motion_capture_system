...
Subject::Subject(std::shared_ptr<rclcpp::Node> nptr, const string& sub_name, const std::string& p_frame)
...
void Subject::processNewMeasurement(const double& time,
                                    ...
                                    std::pair<rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr, nav_msgs::msg::Odometry>& odometry_pair) {
  boost::unique_lock<boost::shared_mutex> write_lock(mtx);
  ...
  odometry_pair = std::make_pair(pub_filter, odom_filter);
